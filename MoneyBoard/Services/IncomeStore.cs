...
/// <summary>
/// 収入（給与）記録データの保持と永続化。PortfolioStore と同流儀（デバウンス＋直列化、
/// 競合時は最新を読み直して通知）だが、ドキュメントが1つなので差分送信はせず全体を保存する（#107）。
/// </summary>
public class IncomeStore(IncomeService svc)
{
    public IncomeData Data { get; private set; } = new();
    ...
    public async Task<bool> LoadAsync()
    {
        ...
        try
        {
            Data = await svc.LoadAsync() ?? new IncomeData();
            IsLoaded = true;
            return true;
        }
