...
public record AccessInfo(List<AccessUserDto> Approved, List<PendingUserDto> Pending, List<string>? EsppEligibleEmployees = null);
...
public class AccessService(HttpClient http, AuthService auth)
{
    ...
    // ESPP 対象社員フラグの ON/OFF。成功時は更新後の承認情報を返す。
    public async Task<AccessInfo?> SetEsppEligibleAsync(string uid, bool value)
    {
        await auth.ApplyTokenAsync(http);
        var resp = await http.PostAsJsonAsync(Path, new { action = "espp", uid, value });
        return resp.IsSuccessStatusCode ? await resp.Content.ReadFromJsonAsync<AccessInfo>() : null;
    }
}
