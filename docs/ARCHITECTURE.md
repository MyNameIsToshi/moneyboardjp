...
- ※ 過去のテストで作られた残置コンテナ `userdata-backup-20260627` が本番に残っていたが、不要のため削除済み
  （2026-07-20・#156）。本番のコンテナは `userdata` のみ。
...
### インシデント対応フロー（検知〜事後）
実際に障害が起きたときに迷わず動けるよう、対応の流れをここに固定する。

1. **検知**: ユーザー（オーナー含む）からの申告、または Application Insights の異常（エラー急増・保存失敗）で気づく。
   自動アラートは未設置（個人開発規模のため。必要になったら別 issue）。
2. **前提確認**: `az login` 済みか確認（トークンは一定期間で失効する。Windows の認証ダイアログが出て進めない場合は
   `az login --use-device-code` を使うとブラウザで完結できる）。
   ```
   az account show
   ```
3. **判断**: 何が起きたかで手法を選ぶ（詳細は下記「復旧手順」）。
   - database／container 自体が消えた → **B. 同じアカウントに復元**
   - データは残っているが中身が壊れている（誤保存・不具合のある移行等） → **A. 新しいアカウントに復元**
4. **⚠️ 巻き戻し時刻の特定と書き込みの停止**（A方式で最も重要・ここを飛ばすとデータを失う）:
   - **いつ壊れ始めたか**を特定する。手掛かりは Azure Portal の「ポイントインタイム リストア」→
     「イベント フィードを検索する」／Application Insights のエラー発生時刻／該当リリースのデプロイ時刻。
   - **特定できたら、まず本番への書き込みを止める**。復元には実測で約10分かかり、その間もアプリは生きていて
     壊れたアカウントへ書き込み続ける。止めずに切り替えると**その間のユーザー入力は復元先に存在せず消える**
     （しかも復元は成功して見えるので気付けない）。具体的な止め方は下記「書き込みの止め方」。
5. **実行**: A か B のコマンドを実行する（`<>` 部分だけ埋める）。
6. **検証**: 復元先で `moneyboard-prod` データベース・`userdata` コンテナが揃っているか確認
   （`az cosmosdb sql database/container list`）。ドキュメント単位の中身まで見たい場合は
   Azure Portal のデータエクスプローラーを使う（`az cosmosdb keys list` での鍵取得は避け、Portal 上で完結させる）。
7. **反映**: 復元したデータを本番へ適用する（丸ごと切替 or 部分コピー）。
8. **事後**: 原因・対応内容を該当 issue にコメント記録。恒久対応が要るなら ADR 化。
   復元用に作った一時アカウントは調査後に必ず削除する（残すとコストが発生し続ける）。
- **この手順は 2026-07-20 に実機で一度通しの訓練（A方式・`moneyboard-cosmos-restoretest` への復元→
  データベース／コンテナの復元確認→削除）を行い、動作することを確認済み**。所要時間の実績は「復元開始から
  アカウント作成完了まで約10分」。

### 書き込みの止め方（復旧作業中の保全）
**Cosmos DB 単体には「読み取り専用モード」は無い**。ネットワーク遮断（`--public-network-access Disabled`）や
キー再生成でも止められるが、いずれも読み取りごと落ちるうえ復帰に手間がかかるため、下記の順で行う。

1. **定期実行を止める（必須）**。本番への自動書き込み源は
   `.github/workflows/record-snapshots.yml`（平日 07:15 UTC の cron → `POST /api/record-snapshots` が
   ポートフォリオのスナップショットを書く）。人が触らなくてもこれは動くので、まずこれを止める。
   ```
   gh workflow disable record-snapshots.yml     # 復旧後に gh workflow enable で戻す
   ```
2. **人による書き込みを止める**。利用者はオーナー＋承認済みの招待ユーザーのみのため、
   現在の規模では周知で足りる。
3. **機械的に完全に止めたい場合**は、SWA アプリ設定の `CosmosDb__DatabaseName` を存在しないDB名に変更する
   （`DataApi.GetContainer()` は毎回この環境変数を読むため、コード変更もデプロイも不要。設定を戻せば復帰）。
   ただし**読み取りも同時に止まる＝実質メンテナンス状態**になり、利用者にはエラーが見える。
   保存失敗時の UI が親切でない点は既知の課題のため、使うなら周知とセットで。

書き込み経路は `SaveData`（家計簿）・`SavePortfolio`・`PostAccess`（承認）・`RecordSnapshots`（定期）の4つ。
1 と 2 で 4 つとも塞がる。

### バックアップ方式
- **Cosmos アカウント `moneyboard-cosmos` は継続的バックアップ（PITR・`Continuous7Days`）を有効化済み（2026-07-20）**。
  - `backupPolicy` は**アカウント単位**の設定で、データベース／コンテナ単位では分けられない。`moneyboard-dev`・
    `moneyboard-prod` は同一アカウント内の別データベースのため、**両方に適用される**（dev 側のみ・prod 側のみに
    絞ることは不可）。
  - 切替前は既定の Periodic（4時間間隔・保持8時間のみ）のままだった。8時間を超える障害・誤削除には対応できず、
    復元にも Azure サポート依頼が必要な粗い運用だったため切替。
  - Periodic → Continuous の切替は**オンライン・一方向**（Continuous → Periodic には戻せない）。サーバーレスアカウントでも
    切替可能（要件確認済み）。
  - 保持期間は 7 日（`Continuous7Days`）を選択。Azure Portal 上の表示でも「このモードは無料で利用できます」と
    明記されている（30日／35日モードはコスト発生）。ただし**無料なのはバックアップの保持であって、復元
    （restore）操作自体には課金が発生する**（Portal の復元画面にも見積もりリンクがある）。訓練目的で
    気軽に繰り返してよいものではない。
  - ⚠️ **「7日分の履歴がある」状態になるのは有効化から7日後（2026-07-27）以降**。継続的バックアップは
    有効化した瞬間からログを取り始めるため、それ以前には遡れない。実際に遡れる下限は
    `oldestRestorableTime` が正（有効化直後の実測値は `2026-07-20T07:35:24Z`＝有効化した時刻そのもの）。
    復元前に必ずこの値を確認すること。
- 確認・変更に使ったコマンド（Azure CLI・要ログイン）:
  ```
  az cosmosdb show --name moneyboard-cosmos --resource-group rg-moneyboard --query "{backupPolicy:backupPolicy}"
  az cosmosdb update --name moneyboard-cosmos --resource-group rg-moneyboard --backup-policy-type Continuous --continuous-tier Continuous7Days
  ```

### 復旧手順（誤削除・データ破損時）
障害の種類によって2つの手法を使い分ける。

#### A. データ破損（誤った上書き・不具合のあるスキーマ移行など）→ 新しいアカウントに復元
`moneyboard-prod` データベース／`userdata` コンテナ自体は存在するが**中身が壊れているケース**。MoneyBoard で
実際に想定される事故はほぼこちら（例: 「スキーマ移行」節の v11 移行のような実データ書き換えでの事故）。
PITR の復元は**同一アカウントへの上書きではなく、指定した時刻の状態を新しい別アカウントとして復元**する
（Cosmos の仕様上、既存アカウントへの in-place restore は不可）。

1. 巻き戻す時刻（UTC）を決める。**下限を必ず確認する**（有効化以前・保持期間外には遡れない）。
   ```
   az cosmosdb restorable-database-account list \
     --query "[?accountName=='moneyboard-cosmos'].{name:name,oldest:oldestRestorableTime}" -o tsv
   ```
   時刻書式は `2026-07-20T11:58:00Z`（末尾 `Z`）で実際に成功している。この形式を使う。
2. 新しいアカウント名で復元を実行する（既存アカウントは変更されない）。
   ```
   az cosmosdb restore --resource-group rg-moneyboard \
     --account-name moneyboard-cosmos --target-database-account-name <復元先の新アカウント名> \
     --restore-timestamp <YYYY-MM-DDTHH:MM:SSZ> --location "Japan East"
   ```
   引数を付けないと**アカウント全体（`moneyboard-dev` も含む）**が復元される。本番だけでよいなら絞る:
   ```
   --databases-to-restore name=moneyboard-prod collections=userdata
   ```
3. 復元先アカウントでデータを確認する（`moneyboard-prod` データベース・`userdata` コンテナ）。
4. 問題なければ、復元先から正しいデータを取り出して本番へ反映する（アプリの接続先を丸ごと切り替えるか、
   壊れたドキュメントだけを復元先からコピーして戻すかは被害範囲で判断）。接続文字列を切り替える場合は
   SWA のアプリ設定 `CosmosDb__ConnectionString`（Azure Portal → SWA → 構成、または
   `az staticwebapp appsettings set`）。**復元先は別アカウントなので接続キーも別物**＝エンドポイントだけ
   差し替えても繋がらない。復元先アカウントのキーで接続文字列を取り直すこと。
   **接続文字列はドキュメントに書かない**。
5. 旧アカウント・復元用アカウントは調査のため一定期間残し、不要になったら削除する。

#### B. 誤削除（database／container 自体を消してしまった）→ 同じアカウントに復元
新しいアカウントを作らず、同じアカウント内に元の名前で復活させる。**消したものに対応する方を実行する
（両方を続けて叩くコマンド列ではない）**。同一アカウント復元は**対象が現存しないこと**が前提のため、
同名のものが残っていると失敗する。

- データベースごと消した場合（配下のコンテナも一緒に戻る）:
  ```
  az cosmosdb sql database restore --resource-group rg-moneyboard --account-name moneyboard-cosmos \
    --name moneyboard-prod --restore-timestamp <YYYY-MM-DDTHH:MM:SSZ>
  ```
- コンテナだけ消した場合（データベースは生きている）:
  ```
  az cosmosdb sql container restore --resource-group rg-moneyboard --account-name moneyboard-cosmos \
    --database-name moneyboard-prod --name userdata --restore-timestamp <YYYY-MM-DDTHH:MM:SSZ>
  ```

#### A・B 共通
- **RPO の目安**: 復元先の時刻粒度自体は細かい（操作単位のログを保持）が、**実際に失うデータ量とは別物**。
  - データ破損（A）では「壊れ始めるより前」まで巻き戻す必要があるため、**破損開始からの経過時間がそのまま
    データ損失**になる（気付くのが遅ければ数時間分）。「直前の書き込みまで戻せる」わけではない。
  - 継続的バックアップには数分程度の遅延があり、「10秒前」のような直近への復元はできない。
  - A は復元先が新アカウントになるため、**切替作業自体にも数分〜十数分のダウンタイム**が発生する
    （B は同一アカウント内に戻すため接続文字列の切替は不要）。
- **担当**: オーナー（Azure サブスクリプション所有者）が az cli で実施。

### リリース前スナップショットの運用
- 通常のリリースでは**追加の手動スナップショットは不要**と判断（PITR が保持期間内の任意時点への復元を代替する）。
  ただしこれは `oldestRestorableTime` 以降しか戻せないことが前提＝**有効化から7日が経つ 2026-07-27 までは
  遡れる範囲が短い**。それまでの間にデータを触るリリースを行うなら、下記の例外扱いにする。
- 例外: **データを書き換えるスキーマ移行を伴うリリース**（`SchemaMigration` で加算的追加ではなく実データを変換する回。
  例: v11／#147）は、リリース直前に `GET /api/data`（要認証）のレスポンス JSON をオーナーが手元に保存する運用とする。
  PITR は「時刻」でしか戻せず、移行直後に別の書き込みが入ると巻き戻し範囲の判断が難しくなるための保険。
  `/release` 手順への機械的な組み込みは行わず、該当するリリースごとに判断する（頻度が低く定型化のコストが見合わないため）。
...
C:\Development\moneyboard\
  ...
  MoneyBoardShared\           ※ 役割は下記「MoneyBoardShared の憲章」を参照
    ...
    StatsMath.cs              統計（グラフ）の純粋ロジック（SelectPeriodYms=期間選択。GraphPage が委譲・v1.3.3。NormalizeCategoryKey=カテゴリ別集計のグルーピングキー正規化・#117で追加。BuildAccountSeries=口座別系列の組み立て（Id 解決・順序保持・同名口座の表示名を連番で一意化）・#157で追加。MonthStartDate=現金/電子マネー支出の合成日「その月の1日」／MergeAmounts=月別推移バケットへのキー別金額合算・#171で追加）
...
```csharp
AppState
  ├─ SchemaVersion              // スキーマ版数（移行判定用・現状 11）
...
| PWA化（manifest・service worker・ホーム画面追加・アプリ更新検知） | ✅ 完了（#76・iOS向けバージョン比較方式#86・常設リロードボタン＋ポーリング検知#132・Web版も含めた更新ダイアログ＋中央レジストリ#141・強制/任意アップデート切替#142） |
...
- **集計軸トグル（請求月/利用月）＋「カテゴリ別/カード別 月別推移」チャート追加（#105）**：カテゴリ別/カード別ドーナツ+一覧の期間メンバーシップは既定で「請求月」（月次ドキュメント`Months[ym].CardDetails`所属＝既存方式）。「利用月」トグルに切り替えると、`CardDetail.Date`（実際にカードを使った日）の年月で判定する方式に変わる（`StatsMath.UsageYmOf`＝純粋関数で`"yyyy-MM-dd"`→`"yyyyMM"`変換）。全月データはクライアント常駐のため、ドキュメント所属とは無関係に全月のカード明細を横断して対象を絞り込んでも低コスト。財布の現金支出（`Debit`）は利用日フィールドを持たないため、このトグルの影響を受けず常に請求月バケットのまま（ユーザー判断・実装前確認）。
  - **新規チャート「カテゴリ別/カード別 月別推移」（横軸=月・積み上げ棒）は当初「常に利用月固定」としたが、実装後のユーザー要望で集計軸トグルに連動する方式へ変更**（設計判断の変更・issueコメントに記録）：`CardDetailsAtBucket(ym)` が `SpendAxis` を見て請求月バケット（ドキュメント所属）／利用月バケット（`CardDetail.Date`）のどちらで各 x 軸点の明細を集めるか切り替える。ドーナツ2枚（カテゴリ別/カード別）と対になるよう、月別推移も2枚（`BuildCategoryTrend`/`BuildCardTrend`）を左右2列（`.graph-row2`）でレイアウト。カード別推移の色はカード別ドーナツと同じ `CardPalette` を表示順に割当。
  - **棒タップの内訳表示は「集計別」（`BreakdownDialog`で項目別合計。カテゴリ別推移からはカテゴリ別合計、カード別推移からはカード別合計）／「明細別」（`OpenTrendDetail`＝既存の`DetailDialog`を再利用し1行ずつ表示。どちらのチャートから開いても同一のバケット内容）をトグルで切替可能**（実装後のユーザー要望で追加）。明細別は閲覧専用（`ShowCategorize=false`）とし、ドーナツ側「選択してカテゴリ設定」の対象範囲は変更しない。`DetailModal`に`SubLabel`（既定=""）を追加し、単月ドリルダウンでは月ラベルを明示（既定""の場合は従来どおり`RangeLabel`＝期間全体を表示）。
  - **集計別/明細別トグルは当初グラフ上部に外置きしたが、グラフの描画自体は変わらずダイアログの中身だけが変わるため、動作確認後のユーザー指摘でダイアログ内蔵のトグルへ移設**：`DetailDialog`/`BreakdownDialog` に任意描画コンテンツの拡張スロット `[Parameter] RenderFragment? ToolbarExtra`（既定null＝両コンポーネントの既存呼び出し元（収入/支出/固定費内訳・カテゴリ別/カード別ドーナツ等）は無変更・無影響）を追加し、`GraphPage` は月別推移タップ由来のダイアログ（`_trendYm is not null` で判定）にだけ `TrendModeToggle()`（`DetailDialog`のソート切替と同じ`.dd-seg`見た目）を差し込む。トグル切替時はダイアログを閉じずに`OpenTrendDrilldown()`で中身だけ再構築する（`_trendYm`/`_trendIsCategoryChart`にタップ元を保持）。`TrendModeToggle()`はRazorテンプレート構文（`@<div>`）を使うため、集計・状態ロジックを集約する`GraphPage.razor.cs`ではなく`GraphPage.razor`側の`@code`ブロックに置く（ロジックは`.razor.cs`、テンプレートのみ`.razor`という例外）。
    - **バグ修正**：モード切替時に切替先（`_detail`/`_breakdown`）だけを設定し反対側を`null`にしていなかったため、動作確認で「ダイアログが二重に重なって表示される」不具合が発覚。`OpenTrendDrilldown()`で切替先を設定する前に反対側を明示的に`null`にするよう修正。
    - **UX修正**：`ToolbarExtra`（トグル）の表示位置が`BreakdownDialog`（合計直下・単独行）と`DetailDialog`（既存のソート切替行に同居→右寄せ）とで揃っていなかった不具合を、`DetailDialog`側も`dd-total-row`直下の専用`dd-tools`行に分離することで統一。
  - **月別推移2枚のホバー時ハイライトは要素（系列＝カテゴリ/カード）単位ではなく月（x軸1点＝積み上げ棒全体）単位にした**：タップ時のドリルダウンは系列に関わらずその月全体の内容を開く仕様のため、ホバーで特定の系列（例：カテゴリ「衣食」の区間）だけが強調されると「そこだけタップすれば衣食の明細が見られる」という誤った期待を与えてしまう（ユーザー指摘）。ApexCharts の `tooltip.shared: true` は月（x軸グループ）単位でホバーハイライト・ツールチップ表示する標準機能のため、`NewBarOptions` に `sharedTooltip` 引数を追加し、この2チャート（`CategoryTrendOptions`/`CardTrendOptions`）にのみ適用（他の積み上げ棒＝収入の内訳推移は対象外・要望のスコープどおり）。各系列の色分け自体（`Colors` 設定）は変更していない。
    - **`tooltip.shared: true` は `tooltip.intersect: false` を伴わないと ApexCharts 側の例外でランタイムクラッシュする**（実機確認で発覚）。`sharedTooltip=true` のときは `Tooltip.Intersect = false` も同時に設定するよう修正。
    - **`sharedTooltip=false` のときは `Shared`/`Intersect` を未設定（null）のままにして ApexCharts 既定（shared=true・intersect=false）を保つ**（コードレビューで発覚）：`Blazor-ApexCharts` の `Tooltip.Shared`/`Intersect` は `bool?` で、未設定なら各既定にフォールバックする。当初 `Shared = sharedTooltip, Intersect = !sharedTooltip` と一律に明示していたため、この共通 `NewBarOptions` を使う既存チャート③「収入の内訳推移」のツールチップが shared（既定）→非shared+intersect に反転する回帰があった。共有時のみ明示設定する形へ修正。
    - **色を持つチャート（ドーナツ2枚＋月別推移2枚）はマスク切替（金額を隠す）でも色を維持する**（コードレビューで発覚）：マスク切替は `RebuildChartOptions` のみ呼び `BuildChartData` を呼ばないため、色を再生成後に引き継がないと `Colors` が null に戻り ApexCharts 既定色になる。ドーナツは元から退避・復元していたが、追加した月別推移2枚（`CategoryTrendOptions`/`CardTrendOptions`）も同様に退避・復元するよう修正（未修正だとマスク切替後だけ推移バーがドーナツと違う既定色になる）。
    - **`states.hover` の既定フィルタ（要素＝系列単位のダークン）が shared のグレー帯（月単位ハイライト）と同時に効き、ちらつく不具合**（実機確認で発覚）。`sharedTooltip=true` のときは `States.Hover.Filter.Type = StatesFilterType.none` で要素単位のフィルタを無効化し、月単位のハイライトのみ残す。
  - 直近の利用月は未請求・未取込の利用が含まれず低く見えることがある旨を集計軸トグルにヒント文で明示（利用月選択時のみ）。既存の「月別支出推移（総支出・請求月ベース・残高連鎖と一体）」は無改修（issue想定どおり）。
- **現金・電子マネーの手入力支出（`Debit`）を、明細ドリルダウンの日付表示・カテゴリ別月別推移でカード明細と同等に扱う（#171）**：`Debit` は実日付を持たず所属月 `ym` しか無いため、2箇所でカード明細（`CardDetail`）と扱いが不揃いだった。
  - **明細ドリルダウンの日付**：現金・電子マネー行（`BuildCategorySpend` の `cashRows`）の表示用日付を、`ym`（"yyyyMM"）そのままではなく `StatsMath.MonthStartDate`（純粋関数・その月の1日を `"yyyy-MM-01"` で合成）に置き換え。`DetailDialog` は無改修で `M/d`（例「7/1」）に整形でき、`DateTime.TryParse` ベースの日付順ソートも実日付として効く。合成は表示用のみで保存データ・入力欄には触れない。
  - **合成日の注記**：`DetailDialog.DetailRow` に `DateIsSynthesized`（既定 `false`）を追加。`true` の行の日付にのみ `CycleInfo` と同型（`:hover`＋`:focus-within`・`tabindex="0"`）の ⓘ ツールチップ（`.dd-date-tip`/`.dd-date-note`・`graph.css`）を表示し、「利用日を記録しないため月初（1日）で表示している」旨をその場に出す。カード明細行には付けない。
  - **カテゴリ別月別推移**：`BuildTrendSeries` に任意の `extraByYm` を追加し、`BuildCategoryTrend` から現金・電子マネー支出のカテゴリ別月別金額（`CashDebitsIn` を月ごとにカテゴリキーで集計）を渡してカード明細分に合算する（合算処理は `StatsMath.MergeAmounts`＝純粋関数）。`BuildCardTrend` は `extraByYm` を渡さないため従来どおりカード明細のみ（`BuildTrendSeries` 共通化の副作用でカード別に漏れない）。現金・電子マネーは利用日を持たないため、軸トグル（請求月/利用月）に関わらず常に所属月バケット（ドーナツ側の `CashDebitsIn` と同じ扱い）。
  - **棒タップ由来の内訳ドリルダウンも現金・電子マネー込みで一致させる（コードレビュー指摘・自動検証で実測して発覚）**：初回実装では `OpenCategoryTrendSummary`/`OpenTrendDetail` が `CardDetailsAtBucket` のみを参照しており、棒の高さ（現金込み）とタップ後の内訳・明細合計（カードのみ）が一致しない不整合が残っていた（Playwright＋Cosmos実データでの自動検証で「棒 ¥100,504 に対し内訳合計 ¥98,004」として実測）。#171 が解消しようとした「ドーナツには出るのに推移には出ない」という不整合を、棒と内訳の間に移しただけの状態だったため修正。
    - `OpenCategoryTrendSummary`：カード明細のカテゴリ別金額に `CashCategoryAmounts(ym, knownCategoryIds)`（`BuildCategoryTrend` と共通の集計）を `StatsMath.MergeAmounts` で合算。棒の高さと内訳合計が構造的に一致する（同じ集計関数を両方から呼ぶため）。
    - `OpenTrendDetail`：**`_trendIsCategoryChart == true`（カテゴリ別推移から開いた場合）のみ**現金・電子マネー行をカード明細行に追加する。行の日付はドーナツ側ドリルダウンと同じく `StatsMath.MonthStartDate` の合成日＋`DateIsSynthesized: true`（ⓘ 注記が出る）。`_trendIsCategoryChart == false`（カード別推移）は現金・電子マネーを持たないため従来どおりカード明細のみ（「カテゴリ別＝現金・電子マネー込み／カード別＝CardDetail のみ」という issue の原則をドリルダウンにも一貫させた）。
    - `OpenCardTrendSummary` は変更なし。
- **期間・集計軸・マスク切替時のグラフカードのちらつきを解消（#151）**：期間/集計軸/マスクいずれかを切り替えると各チャートの `@key`（`PeriodKey`/`SpendAxis`/`_maskRev` を含む）が変わり、Blazor が `<ApexChart>` を破棄→再生成する。再生成が完了するまでの一瞬チャートの DOM が消えて格納先の `.graph-card` が縮み、元に戻ることで裏のカードがガタついて見えていた。7枚のチャート（コンボ・収入内訳・口座残高推移・カテゴリ別/カード別ドーナツ・カテゴリ別/カード別月別推移）それぞれの `<ApexChart>` を、`min-height` を持つラッパー `div.chart-box`（`graph.css`。高さは CSS変数 `--chart-h` をインラインで指定）で囲み、再生成中も周囲の寸法を維持するようにした（当初は予約値を `Chart.Height` そのまま＝300/240/340px にしたが、これが誤りだった経緯と最終的な予約値は下記「第三のガタつき要因」を参照）。ドーナツ2枚（`SpendBreakdownCard.razor`）は既存の `.spend-chart` に `chart-box` を併用。予約は `min-height` の下限のみで幅は `width:100%` 流動のまま（スマホ幅でも同じ値で有効。`Chart.Height` は固定pxでレスポンシブ未使用のため全幅で同一高さ＝スマホでも一致）。データなし（`else` の `graph-placeholder`）側は元々チャート自体が存在せず再生成も起きないため対象外。
  - **上記だけでは解消しきらなかった第二のガタつき要因（実機確認＋Playwrightでの高さ計測で発覚）**：カテゴリ別/カード別支出の「ドーナツ＋一覧」カード（`SpendBreakdownCard.razor`）は、期間/集計軸トグルで対象カテゴリ・カード数が変わるたびに一覧（`.spend-list`）の行数が変化し、ドーナツ側の `min-height` 予約とは無関係に**カード全体が最大100px規模で伸縮**していた（`@key` によるチャート破棄/再生成とは別原因＝一覧の行数という実データ差に起因）。既存の明細ドリルダウン（`DetailDialog`）の `.dd-list` が採る「一覧本体を `max-height`＋`overflow-y:auto` の内部スクロールにする」パターンをそのまま踏襲し、一覧の行部分だけを新設の `.spend-rows`（見出し行・合計行は対象外＝常に全行表示のまま）でラップして `max-height:300px` を設定。行数がどれだけ変わってもカードの外寸は変化せず、収まりきらない分はカード内部でスクロールする。
  - **第三のガタつき要因（マスク切替でも発生していたことから発覚）**：上記2点を直した後もユーザーから「期間・マスク・集計軸のいずれも、まだ違う出方でガタつく」と再指摘があり、Playwrightで実際にアプリを起動し `requestAnimationFrame` 毎に `.graph-card`/`.chart-box` の実測 `getBoundingClientRect().height` を1フレーム単位で記録して原因を切り分けた。判明したのは、`.chart-box` の予約値を `Chart.Height` の設定値そのまま（コンボ340px・ドーナツ以外の棒/線4枚240px）にしていたのが誤りだったこと：ApexCharts は凡例(Legend)を持つ棒/線グラフを実際には設定値より**約15px大きく**描画する（ドーナツは凡例込みで設定値どおりに収まる）ため、`@key` 変更（マスク切替も `_maskRev` を含むため対象）で破棄→再生成されるたびに、実高さ(355/255px)から予約値(340/240px)まで一旦本当に縮み、再生成完了時にまた実高さへ戻るという**本物の縮小→復帰**が起きていた。予約値を実測した安定時高さ（コンボ355px・ドーナツ以外の棒/線4枚255px・ドーナツ2枚は元々300pxで正しかったため変更なし）に合わせて修正。修正後はPlaywrightのフレーム単位計測で、マスク/期間/集計軸いずれのトグルでもチャートカード（`.chart-box`）側の高さが完全に一定（0px変動）になったことを確認した。ドーナツ＋一覧カードのみ、期間/集計軸で対象件数が変わる分の正当な（かつ`.spend-rows`で上限を設けた）サイズ差が残るが、フレーム単位で見ても縮小→復帰のような不自然な動きはなく、1回のクリーンな遷移になっている。
  - **既知の残存差異**：`Chart.Height` に対する凡例分の加算値（+15px）は今回計測した画面幅（`.graph-row2` 2列時で概ね570〜600px）での実測値であり、凡例の折返し行数は列幅に依存するため、大きく異なる幅（極端に狭いスマホ幅等）では凡例が2行になり、この加算値だけでは足りず微小な差異が再発する可能性が残る（`Chart.Height` は固定pxで幅非依存という前提自体はドーナツ/カード配置には成立するが、凡例の折返し行数までは幅非依存ではないため）。実害が確認された場合は別途対応する。
- **同名口座があると②口座別月末残高推移で例外が発生する不具合を修正（#157）**：`BuildChartData` が `Svc.ActiveAccounts.ToDictionary(a => a.Name, …)` で口座名をキーにしていたため、同名の口座が2つ以上あると `ArgumentException`（重複キー）が投げられ、`Load()` に try/catch が無いことから `OnInitializedAsync` が未処理のまま抜け、統計ページ全体が描画されなくなっていた。**修正**：`BalanceSeriesData` を `Dictionary<string, List<ChartPoint>>` から順序付きの `List<(string Name, List<ChartPoint> Data)>` へ変更し、残高は一意な `Account.Id` で解決・表示名は組み立て時に一度だけ確定する（`StatsMath.BuildAccountSeries`＝純粋関数として `MoneyBoardShared` 側に切り出し）。
  - **Dictionary をやめて List にしたのが要点**：この系列データはキー引きされず `@foreach` で列挙されるだけで、かつ**列挙順が色割当（`BalanceLineOptions.Colors` は `ActiveAccounts` 順で生成）と 1:1 で対応する必要がある**（ApexCharts は i 番目の系列に `Colors[i]` を割り当てる）。`Dictionary` の列挙順は .NET の仕様として保証されないため、キーを `Id` に替えるだけでは「重複キー例外は消えるが系列と色の対応は暗黙の実装依存のまま」になる。`List` にすることで並び順が構造的に保証され、同名口座でも破綻しない状態が設計として担保される。
  - 表示名は `BuildChartData` 時点で解決して系列に持たせる（`.razor` 側で `Svc.AccountName()` を都度引くと、`State.Accounts` の線形探索が再描画のたびに系列数ぶん走るため）。
  - 口座名へのユニーク制約は、既存データに同名口座が存在しうるため採らない方針（受入条件どおり）。
  - **凡例の同名衝突を連番で解消**：本番環境（v2.18.1・修正前）で実際に同名口座（既存口座のリネームで再現）を作って再現・修正確認を行ったところ、ページは正しく開き2系列とも独立した値で描画される一方、**ApexCharts の凡例ホバーハイライトが常に1件目の系列にしか反応しない**（凡例クリックでの表示/非表示トグルは系列インデックスで正しく解決するが、ホバーは系列名で解決するため名前が衝突すると1件目に固定される）ことが判明。一度は「ユーザー自身の責任として表記はそのまま・ホバーの紛らわしさは許容する」方針で連番を撤回したが、動作確認で実際にホバーの先勝ちを目にして**やはり解消してほしいとの判断で連番方式に戻した**（issueコメントに経緯を記録）。`BuildAccountSeries` で既出の表示名を検出し、2件目以降にのみ ` (2)`, ` (3)` … と連番を付けて一意化する（名前が重複していない口座は影響を受けない）。連番で作った名前が別口座の実名（`楽天` ×2 に加えて `楽天 (2)` という口座が実在する等）と衝突する場合は、未使用の名前に行き着くまで番号を送るため、一意性は入力の口座名によらず保たれる。口座番号（`Account.AccountNumber`）による区別は、入力 UI・表示のどこからも使われておらず廃止済みのパラメータのため不採用。
  - **口座名・カード名の重複そのものを作成/改名時に防ぐ案は別 issue とする**：クライアント側の検証だけでは、複数クライアント間の同時編集（同時に別々の口座を同じ名前へリネーム等）をすり抜けて重複が生まれ得る（直近の `#154`＝複数クライアント間のスキーマ整合性の不具合修正が示すとおり、この種の競合は実際に起き得る）ため、たとえ将来「作成時に重複を弾く」対応を入れても、本対応（`Account.Id` での解決＋表示名の連番一意化）は防御としてそのまま残す。カード名は `CardId` で管理されており重複してもクラッシュしない（見分けにくいだけの UX 課題）ため、新 issue のスコープでは口座名ほど緊急ではない。
  - **`BalancePalette` を4→6色に拡張（ユーザー判断）**：4色ローテだと口座5件目から色が循環し、同名でなくても別口座同士が同色になり得た。紫（`#6b4c9a`）・青緑（`#1f7d7a`）を追加し6色ローテに変更。
...
### 設計判断の記録（ADR）
- **明細ドリルダウンの「選択してカテゴリ設定」を実装（#118）** … #44 で UI の枠（ボタン表示）のみに留めていた実処理を実装。未分類の明細は複数月にまたがるため、既存の一括カテゴリ（`CardTab` の店名グルーピング・当月限定）とは別に、`DetailDialog` 側で**行単位のチェック選択→カテゴリ選択→適用**という単純な方式を採用（店名グルーピングや前方一致ルール記憶は対象外＝スコープを絞る）。
  - **月をまたぐ反映**：`DetailDialog.DetailRow` に `Id`（既定 `""`・カード明細=`CardDetail.Id`／現金支出=`Debit.Id`）を追加し、`GraphPage.BuildCategorySpend` が「未分類」カテゴリの明細を組み立てる際に `Dictionary<string, Action<string?>> _categorizeTargets`（行Id→実体へのsetter）を同時に構築。選択行の Id をこの辞書で引いて対象の `CardDetail`/`Debit` を直接書き換えることで、月ごとに保存先が分かれる構造でも1回の適用で複数月へ反映できる。
  - **反映後の表示**：適用後は `Svc.SaveAsync()`＋`BuildChartData()` で再集計し、開いている「未分類」ドリルダウンを最新件数で更新。選択分がすべて分類済みになり対象0件になった場合はダイアログを自動的に閉じる。
  - **選択モードのUI配置は既存の PC/スマホ分岐パターンを踏襲**：トリガーボタン（`dd-cat-top`＝PCのみ／`dd-action-bar`＝スマホのみ）は既存の二重配置のまま、選択モード時は同じ枠内で操作群（全選択・件数・カテゴリ select・適用/キャンセル）に差し替える（新規ブレークポイントを増やさない）。
  - **見出しの表記を実態に合わせて修正**：「カテゴリ別 支出（カード明細）」は #77 で財布の現金支出も合算対象に加えて以降、名実が一致していなかったため「カテゴリ別 支出」に変更（空データ時のメッセージも「カード明細・現金支出がありません」に修正。`CategorySettings.razor` の既存表現に合わせた）。「カード別 利用額」はカード専用の切り口のまま変更なし。固定費まで含めた全支出のカテゴリ化は、固定費マスタ・展開ロジック・UI全体への波及が大きいため見送り、現状の対象（カード明細＋財布の現金支出）を維持する判断とした。
  - **#77 由来の不具合を同梱修正（財布の現金支出が「未分類」のまま統計に一切出ない）**：本対応の実装中、`GraphPage.CashDebitsIn` が `CategoryId` の非空チェックで対象を絞り込んでいたことが判明。財布で現金支出を新規追加した直後は `Debit.CategoryId` が `null`（UI上は「未分類」が選択されて見えるが、ユーザーがカテゴリ select を一度でも操作しない限り値は `null` のまま）のため、この絞り込みに引っかかって永久に集計から除外されていた（#77 の「本機能の主眼」＝現金支出をカテゴリ別支出グラフへ反映する、が実質機能していなかった）。カード明細の未分類はグラフ・ドリルダウンに正しく現れるため、この非対称に気づいたのが発端。**修正**：`CashDebitsIn` を「`CategoryId` が空でない」判定から「口座が財布（`Account.Type == AccountType.Wallet`。ソフト削除済みも過去月参照のため対象）」判定に変更。財布のledgerには手入力の現金支出しか載らない（固定費・カードは #124 で財布を引き落とし口座に選べないため）ため、口座で絞り込めば `CategoryId` の値によらず取りこぼしなく列挙でき、未分類（null/空/参照切れ）の現金支出も `NormalizeCategoryKey`（#117）で他の未分類と同じ「未分類」キーに正規化されて反映されるようになった。
...
- **統計画面のレイアウト/質感リデザイン＋グラフ明細ダイアログ刷新（#44）** … 外部 Claude Design の設計（`stats-spec.md`＝採用「案A 指標バンド型」／`dialog-spec.md`＝PC「案A モーダル」・スマホ「案C ボトムシート」）を採用し、月次（#43）と同じデザイン言語へ。**集計・期間処理・ドリルダウンのロジックは不変**。
  ...
  - **スコープ外（別issue）**：「選択してカテゴリ設定」は当初 **UI の枠まで**（複数選択→一括カテゴリ更新の実処理は未実装＝プレースホルダ）に留めた。実処理は #118 で実装済み（下記参照）。突出判定アルゴリズムの精緻化・CSV書き出しも別issue（dialog-spec §8）。
...
- **資産（ポートフォリオ）タブのレイアウト/質感リデザイン（#45）** … Claude Design の PC モック（`MoneyBoard Assets.dc.html`／`docs/assets-spec.md` 相当）を採用し、月次（#43）・統計（#44）と同じデザイン言語へ。(1) **総資産をダークのヒーロー化**（含み益/含み損ピル＋**元本/評価損益(率)/当日損益をヒーローに集約**。`.summary/.stat/.hero` を monthly.css から流用）。**当日損益**＝全銘柄前日比の円合計は新規集計、(2) **資産クラスごとにグループ化＋小計**（評価額＋損益小計・米国株のみ円/ドルトグル）、(3) 市場指数（PC＝ヒーロー右に2段＝3+2／スマホ＝ヒーロー下に横スクロール小カード）、(4) 構成ドーナツは**中央に総資産 total（万単位）**＋一覧集約・凡例OFF、(5) 絵文字を Material Symbols（savings/refresh/add/tune/close/drag_indicator/trending_up・down/keyboard_arrow_up・down）へ。**集計・価格取得・USD/JPY換算・ドリルダウン・推移ロジックは不変**。
  ...
  - **要約ヒーロー＋指数までをスクロール追従で固定**：当初トップバーのみ `.pf-sticky` だったが、ユーザー要望で総資産ヒーロー＋指数ごと `.pf-sticky` に内包（リデザイン前の「総資産＋指数が固定」挙動を踏襲）。固定ヘッダー上端にスクロール時の隙間が出ないよう `.pf-sticky` に `padding-top` ＋同量の負 `margin-top` で背景(paper)を被せる。**この量は `.wrap` の `padding-top` と一致させる必要がある**：`24px` の当て込み（実際は `20px`）で数px 多く、スマホ（`.app-scroll` が `overflow-y:auto`＝sticky の基準）では静止時から sticky のクランプが働いて中身が下にずれて見える不具合があった（#146）。修正時、値のハードコード二重管理（`.wrap`側とpf-sticky側）による再発を防ぐため、共通の CSS 変数 `--wrap-pad-top`（base.css の `:root`）に一本化し両者から参照する形にした。統計 `graph-sticky` はこのトリックを使わず素の margin のみのため、この種のズレは起きない。
...
- **PWA化の実装方式（#76・#67の実装）** … Blazor WASM SDK 標準の `ServiceWorker` MSBuild 連携（`MoneyBoard.csproj` に `<ServiceWorkerAssetsManifest>` プロパティ＋ `<ServiceWorker Include="wwwroot\service-worker.js" PublishedContent="wwwroot\service-worker.published.js" />`）を採用。追加 NuGet パッケージ・ビルドスクリプト変更は不要（`dotnet publish` 時に `service-worker.js` が published 版へ差し替わり、`service-worker-assets.js`＝静的アセットのハッシュ一覧が自動生成される）。
  ...
  - **使用中の更新検知＝常設リロードボタン＋3分間隔ポーリング（#132）**：#86 は起動時（前回バージョンとの事後比較）にしか気づけないため、アプリを開いたまま長時間使い続けるケースをカバーできなかった。Web Push は #67 で不採用済み（iOS非対応・実装コスト大）、SWライフサイクルイベントも #86 で信頼できないと判明済みのため、いずれにも依存しない方式にした。
    - **軽量ポーリング対象＝`wwwroot/version.json`（ビルド時に`<Version>`から自動生成）**：`announcements.json`と同様、素の静的JSONファイルは publish 出力に含まれる全ファイルが`service-worker-assets.js`のプリキャッシュ対象マニフェストに載るため、そのままでは常にキャッシュ済みの内容が返ってしまう（issue想定の懸念どおり）。`MoneyBoard.csproj`に追加した`WriteVersionJson`ターゲット（`BeforeTargets="BeforeBuild"`・`WriteLinesToFile`のみ・追加パッケージ不要）で`version.json`を生成する。**注意（コードレビューで判明）：このファイルがプリキャッシュ対象マニフェストに載るか否かはビルド状態に依存する**——クリーンビルド（CI/本番 publish は毎回 fresh checkout＝`version.json`は`.gitignore`済みで未生成）では wwwroot 静的アセット列挙の時点で存在しないためマニフェストに載らないが、ファイルが残っている状態での再ビルド（ローカルの反復 publish 等）では列挙に拾われてマニフェストに載る（実測で確認：eval時に不在→0件／存在→1件）。したがって「生成タイミングでマニフェストから除外される」ことには依存できない。堅牢性は取得側で担保する：`AppUpdateService`はポーリング時に`version.json?t=<epoch>`とクエリを毎回変えて取得するため、`service-worker.published.js`の`onFetch`（`cache.match()`）はクエリ差でmissして必ずネットワークへ抜け、ブラウザHTTPキャッシュも回避される。内容は`{"version":"vX.Y.Z"}`で`<Version>`が単一情報源（手動同期するファイルを増やさない）。生成物のため`.gitignore`済み（コミット不要）。
    - **ポーリング・比較ロジックは`AppUpdateService.PollForUpdateAsync`（3分間隔）**：`AppVersionMath.ShouldNotifyUpdate`（#86で導入済みの「2つのバージョン文字列が異なるか」判定）をそのまま再利用し、実行中バージョン（`AppVersionProvider.Current`）とポーリング結果が異なれば`UpdateAvailable`を立てる。新規の純粋ロジック追加はなし（既存関数の再利用のみのためテスト追加は不要）。検知後はそれ以上ポーリングを続ける意味がないためループを終了する。取得失敗（オフライン等）はその回だけスキップし次回間隔で再試行する。
    - **常設リロードボタンはPCでは`AppTitle`から分離し`SideNav`の独立ナビ項目にする（実装後のPC実機確認で修正）**：当初issue通り`AppTitle`にそのまま常設したところ、PCの`SideNav`ブランド部（`sidenav-brand`）は幅が狭く`.app-title`の`flex-wrap:wrap`でボタン分がタイトル行を折り返させてしまい、サイドバー全体が1行分下にずれる不具合が実機確認で見つかった。ベル（#38）が同じ理由で独立ナビ項目へ分離された前例を踏襲し、リロードボタンも`ShowReload`パラメータ（`ShowBell`と対称・既定true）を追加してPCの`AppTitle`呼び出しだけ`false`にし、`SideNav`側に「更新」ナビ項目（「金額を隠す」の下）として独立配置した。スマホは元々`AppTitle`が唯一のヘッダーのため従来どおり`AppTitle`内に表示する。バッジ（`.reload-dot`）は既読件数を持たないため`.bell-badge`と別に単純な丸のみのCSSを追加し、`SideNav`側では既存の`.sn-ico-wrap`（アイコン基準の絶対配置コンテナ・#38で導入済み）にそのまま乗せて再利用した。
    - **常設リロードボタンはPWA（ホーム画面追加後のスタンドアロン起動）時のみ表示（実装後のフィードバックで追加）**：通常のブラウザタブではブラウザ自身のリロード操作が既にあるため、常設ボタンは主にPWA起動時（ブラウザUIが無い）に価値がある。`pwa.js`に`moneyboardPwa.isStandalone`（`matchMedia('(display-mode: standalone)')`、iOS Safari向けに`navigator.standalone`をフォールバック）を追加し、`AppUpdateService.InitAsync`で一度だけ取得して`IsStandalone`に保持する（起動中に変化しないため`ViewportService`のような変化イベント購読は不要）。`AppTitle`・`SideNav`はともに`AppUpdateSvc.IsStandalone`を見てボタン自体の描画有無を切り替える。
    - **ボタン押下時の処理はpwa.jsの既存SKIP_WAITINGフローを再利用**：ブラウザの自動更新チェックは前回チェックから24時間未満だとスキップされる仕様のため、押下時は`registration.update()`で明示的に再チェックしてから`registration.waiting`を確認し、見つかれば`activateWaitingAndReload`（pwa.jsトーストの「更新する」クリックと同一処理に共通化）でSKIP_WAITING→リロード、見つからなければ（既に最新・SW未登録等）通常のリロードにフォールバックする。ただし`update()`は新SWの install 開始時点で resolve されることが多く直後は`registration.waiting`がまだ null のことがある（そのまま判定すると更新を取りこぼし旧版のまま再読込になる）ため、`registration.installing`が`installing`状態の間は`statechange`で installed(=waiting) へ遷移するまで待ってから判定する（install 失敗/長期化に備え10秒でタイムアウト・コードレビュー指摘#2で追加）。`activateWaitingAndReload`は既存の`mb_pwa_manual_reload`フラグ（クリック時刻つき）を立てるため、リロード直後に#86の事後通知（「vX.Y.Zに更新されました」）と二重に出ることはない（既存の抑止機構をそのまま利用・新規のフラグ追加なし）。
  - **使用中の更新検知＝Web版も含めた更新ダイアログに拡張、通知手段はバッジからダイアログへ変更（#141）**：#132はPWA限定のバッジ通知として確定していたが、通常ブラウザタブのユーザーにも更新に気づいてほしい・バッジより明確な導線が欲しいという要望を受け拡張した。ポーリング（`AppUpdateService.PollForUpdateAsync`）はPWA限定（`IsStandalone`ゲート）だったのを撤去し常時実行にした（検知後のバッジ表示先がPWAのリロードボタンのみだった制約が無くなったため）。バッジ（`.reload-dot`）は廃止し、`AppTitle`/`SideNav`の常設リロードボタン自体（#132のまま）は残す。バッジ表示のためだけに購読していた`AppTitle`/`SideNav`の`AppUpdateSvc.Changed`も、バッジ機能自体を今後追加復活させる予定が無いため（コードレビュー指摘・実質デッドコード化）削除した（両コンポーネントが読む`AppUpdateSvc`の値は起動時固定の`IsStandalone`のみになり、`Changed`購読は不要）。
    - **「操作中でないか」の判定＝中央レジストリ方式**：ダイアログを編集操作中にいきなり出すと未保存データを損なう・作業を妨げるおそれがあるため、`MoneyBoard.Services.OverlayRegistryService`（モーダル/シート/ビジー状態の中央レジストリ）を新設した。各タブ/ページは既存の背面スクロールロック判定（`AnyDialogOpen`。#132以前から各タブに存在した仕組み）をそのままここへも `Overlay.SetOpen(nameof(Xxx), AnyDialogOpen)` の形で報告する。issue時点の想定は「ConfirmDialog/WarnDialog/BottomSheet/DetailDialog等の共通コンポーネント側で登録する」方式だったが、実装時に調査したところ、アプリ内の全ダイアログ（生の`.dialog-overlay`直書き含む）は既にこの`AnyDialogOpen`パターンで漏れなくスクロールロックに使われていることが分かった（AccountsTab/CardSettings/CategorySettings/CardTab/FixedCostTab/FixedIncomeTab/Portfolio/GraphPage/MonthlyTab/MyPageTab/MainLayoutの11箇所）。共通コンポーネント側で登録する方式は生の`.dialog-overlay`個別対応と二本立てになり、登録漏れのリスクも増えるため、既存の`AnyDialogOpen`を再利用する方式へ変更した（実装方式の変更をissueコメントに記録）。CardTabのAI読取busy（`_shotCardId`）・一括カテゴリのAI分類busy（`ShowBulk`中の`_classifyBusy`）も、それぞれの状態が立っている間は対応する`AnyDialogOpen`が真になるため、この仕組みだけで自動的にカバーされる。0件⇔1件以上の遷移でのみ`Changed`を発火する`SetOpen`は新規の純粋ロジックのため、新設した`MoneyBoard.Tests`（フロント側の純粋ロジック専用。既存の`MoneyBoardShared.Tests`/`MoneyBoardApi.Tests`と同じくCIの`dotnet-test.yml`で実行）にユニットテストを追加した（コードレビュー指摘）。
    - **未保存のインライン編集＝`AppStateStore.HasPendingChanges`**：`RequestSave()`で true、デバウンス保存（`SaveAsync`。成功/競合/失敗いずれの完了時も）で false にする新規プロパティ。即時保存（`Save()`から直接`SaveAsync`を呼ぶ既存の各所の呼び出し）は対象外（issue記載どおり`RequestSave`起点のみ）。
    - **表示判定はイベント駆動**：ポーリングで更新を検知しても、操作中（`AppStateStore.HasPendingChanges`または`OverlayRegistryService.IsAnyOpen`）なら即座には出さず、両サービスの`Changed`イベント（＝保存完了／オーバーレイのclose）を購読して安全になったタイミングで再評価する（一度きりのチェックにしない）。
    - **一度きりの表示**：「あとで」を選ぶと`AppUpdateService.DismissUpdateDialog()`が呼ばれ、以降はこのセッション（アプリ再起動＝ページ再読込まで）は再表示しない。永続化はしない（再訪すれば改めて通知してよいため）。
    - **既存トースト（#86の起動時トースト・pwa.jsのSWイベント発火時トースト）との重複回避**：#86の「vX.Y.Zに更新されました」は更新後（リロード後）に出るため新ダイアログ（更新前に出す）と時間的に競合しない。pwa.jsの`updatefound`/`registration.waiting`検知による自動トースト（`showUpdateToast`）は、常時ポーリングになった新ダイアログと同じ更新を二重に知らせる可能性があるため削除した（`activateWaitingAndReload`・`checkForUpdateAndReload`など実際のリロード実行部分は新ダイアログの「更新する」ボタンからも再利用するためそのまま残す）。あわせて`#pwa-update-toast`関連CSSと、二段重ね用の`body:has(#pwa-update-toast)`ブリッジも削除した。
  - **リリースごとの強制/任意アップデート切替（#142）**：重大な修正・互換性を壊す変更など即時更新してほしいリリースのために、`MoneyBoard.csproj`に`<Version>`と並べて`<ForceUpdate>`（既定`false`）を追加した。`WriteVersionJson`ターゲットが`version.json`へ`forceUpdate`フィールドも書き出し（`<Version>`と同じ単一情報源パターン）、`AppUpdateService`の`VersionPayload`がこれを読み取って`IsForceUpdate`に反映する（ポーリングで検知した`_pendingVersion`と一緒に`_pendingForceUpdate`として保持し、ダイアログ表示時に確定させる）。`forceUpdate=true`の間は`AppUpdateDialog`が「あとで」ボタンを描画せず「更新する」のみになる（背景クリックで閉じない既存方針はそのまま）。「操作中でないか」の判定（中央レジストリ＋`HasPendingChanges`）は強制時も変更していない——issueの受入条件はダイアログのボタン構成のみを対象としており、安全なタイミングまで待つ既存ガードを外す事故コストの方が大きいため踏襲した。
    - **JSON への書き出しは小文字へ正規化する**：MSBuild のブール値は大小文字を区別しない（`True`/`TRUE` も可）が、JSON のブールは小文字 `true`/`false` のみ。`$(ForceUpdate)`をそのまま埋め込むと大文字表記や未定義（空）で不正な JSON になり、`AppUpdateService`の`GetFromJsonAsync`が例外→`catch`で握りつぶされて以降そのセッションの更新検知が恒久停止してしまう。`WriteVersionJson`は`$(ForceUpdate.ToLowerInvariant())`が`true`のときだけ`true`、それ以外は`false`へ正規化してから書き出す。
    - **既知の制約（許容）：同一セッション中の2回目リリースはリロードまで反映されない**。ポーリング（`PollForUpdateAsync`）は`while (_pendingVersion == null)`で最初の1件を検知した時点で停止する#141由来の設計のため、あるセッションで任意更新を検知・却下した後にさらに強制更新をデプロイしても、そのセッションでは検知されず（pending を強制へ昇格もしない）、次のリロード／再起動まで届かない。踏むには「セッションを開きっぱなし」×「その最中に2回目のデプロイ」が両方必要で、ワーストケースも「強制更新が次回リロードまで遅延」に留まる（新しいセッションでは確実に効く）。修正には#141の「1セッション1回検知」設計自体の変更が必要で、確実に効いている挙動へのリスクに見合わないため実装は見送る。
  - **iOS 16.4 未満のスタンドアロン対応**：manifest の `display:standalone` は iOS 16.4+ でのみ有効なため、`index.html` に旧来の `apple-mobile-web-app-capable`（`content="yes"`）を追加。`apple-mobile-web-app-status-bar-style` は `black-translucent`（透過）だと `viewport-fit=cover` とヘッダー側の `env(safe-area-inset-top)` 対応が別途必要になり、本アプリは bottom 側の safe-area（botnav）のみ対応済みで top 側は未対応のため、既存レイアウトと衝突しないよう `default`（不透明）を採用。※ `default` はステータスバー領域を不透明に確保しコンテンツをその下から描画するため、後述 #149 で `viewport-fit=cover` を追加した後も top 側の扱いは変わらない（`viewport-fit=cover` と top の `env(safe-area-inset-top)` 対応が対で必要になるのは `black-translucent` 選択時のみ）。
...
- **財布（現金）＝既存 `Account` を拡張した特殊口座＋ATM対称実体化（materialize）（#77）** … 現金の手元残高・使い道を追跡するため、独立エンティティではなく既存 `Account` にフラグを追加する方式を採用（残高連鎖・台帳構造・収入/支出ゾーンをそのまま再利用でき、固定費#87と同じ「既存モデルへのフラグ追加」路線を踏襲）。アクティブな財布（`Type == AccountType.Wallet && !IsDeleted`）は同時に1個のみ・任意作成。
  - **口座種別の enum 化（#147）**：口座種別を今後増やす（電子マネー・将来の他種別）にあたり bool フラグ乱立を避けるため、当初の `Account.IsWallet`（bool）を `Account.Type`（`AccountType { Normal, Wallet, EMoney }`。本 issue では Normal/Wallet の2値のみ使用）へ一本化する挙動不変リファクタを実施。スキーマ v11 で既存の `IsWallet==true` を `Type=Wallet` へ変換する。`IsBonusAccount` は種別と直交する属性のため対象外（フラグのまま存置）。**判定の正本は `Type` で、アプリロジックは `IsWallet` を参照しない**が、旧フィールド自体は削除せず `Type` の別名として書き戻しも維持する（理由と削除時期は「スキーマ移行」節の v11 を参照）。
...
- **Cosmos DB を継続的バックアップ（PITR・`Continuous7Days`）へ切替、復旧手順を確立（#156）** … 本番アカウント
  `moneyboard-cosmos` は既定の Periodic（保持8時間のみ）のままだったことが判明し、Continuous7Days へ切替
  （オンライン・一方向。7日より安価な30日／35日モードは非選択）。復旧はデータ破損時「新しいアカウントに
  復元」・誤削除時「同じアカウントに復元」の2手法を使い分ける設計とし、実際にテストリストア（実測約10分）
  を行って動作確認済み。復旧作業中に本番への書き込みを止める手段が Cosmos 単体には無いことが判明したため、
  自動書き込み源（`record-snapshots.yml` の cron）を止める運用を暫定策として定めた。恒久的なメンテナンス
  モードの設計は別 issue（#167）で検討する。詳細は「運用・障害復旧」節を参照。
...
### 期間・構成トグル・マスク切替時のポートフォリオカードちらつきを解消（#153）
- #151（GraphPage の同種修正。詳細は「統計ページ（GraphPage）」節の ADR）と同じ原因・同じ対策パターンを Portfolio.razor に横展開。資産構成ドーナツ（`comp-*`。クラス別/銘柄別・PC は2枚同時描画、スマホは構成トグルで1枚）と、総資産・元本推移／評価損益推移（`asset-*`/`pnl-*`）の各 `<ApexChart>` を `.chart-box`（`graph.css`。`min-height` を CSS変数 `--chart-h` で予約）で囲み、`@key`（`_compRev`/`_trendRev`/`_maskRev` を含む）変化時の破棄→再生成によるレイアウトシフトを防止。
- 予約値は #151 で確立した方式（凡例なしのドーナツ＝`Chart.Height` そのまま／凡例ありの棒・線グラフ＝実測+15px）を踏襲：資産構成ドーナツは `Legend.Show=false`（凡例は右の一覧に集約）のため `Chart.Height=260` のまま `--chart-h:260px`。総資産・元本推移／評価損益推移は凡例を明示OFFにしていない折れ線（`NewLineOpt`）のため `Chart.Height=220` に+15pxして `--chart-h:235px`。**#151 のような Playwright フレーム単位計測による実測検証はこの issue では実施していない**（+15px 則の横展開のみ）。残存するガタつきが実機確認で見つかった場合は #151 と同様の手順（フレーム単位計測）で再調整する。
- #151 の「第二のガタつき要因」（一覧行数差によるカード外寸への波及）も同様に発生しうるため、資産構成の一覧（`spend-list` 内の行部分）を `.spend-rows`（`max-height:300px` の内部スクロール）でラップ。特にスマホの構成トグルは「クラス別」（資産クラス3種で固定）と「銘柄別」（保有銘柄数に依存し可変）を切り替えるため、この対策がないとトグルのたびにカードが伸縮する。

### 保有ゼロの銘柄（売却済み）を一覧から分離（#178）
- **背景**：全数量を売却した銘柄も `Holding` が残り続け、数量0・元本0・評価額0の行として保有中一覧に居座っていた。「削除」は物理削除（買付・売却・配当の記録ごと消える＝実現損益の履歴が失われ #175 の売却履歴と矛盾）のため使えず、データは残したまま一覧から分離する方式を採った。
- **売却済みはフラグを持たせず現在数量からの派生状態として判定**（`PortfolioMath.IsHeld(qty) => qty > 0`）。`Holding` へのフィールド追加は行わない。理由は2点：①買い戻し（同名銘柄への新規買付追加）で自動的に保有中へ戻る＝別途「復帰」処理が不要、②スキーマ追加に伴う移行・保存ガード（#136/#137 で事故った設定/月次データ追加と同種のリスク）を負わない。数量が負（売り数量が買付を超える入力ミス）も同じ判定式で売却済み扱いになる（一覧では赤字＋⚠で入力ミスとして視認できるようにする）。
- **一覧描画・並べ替え・価格自動更新の対象を保有中のみに統一**：`Portfolio.razor.cs` の `Ordered`（従来は `!IsDeleted` のみで数量0/負を素通ししていた）を `IsHeld` でも絞り込み、D&D・▲▼並べ替え・`UpdatePrices` の取得対象銘柄すべてに波及させた（数量集計自体は #178 以前から数量0を除外済みで正しかったため計算ロジックは不変）。売却済みは新設の `SoldHoldings`（最終売却日の新しい順）で別リスト化し、保有銘柄一覧の下に折りたたみ「売却済み（N件）」セクションとして表示（銘柄名・口座バッジ・最終売却日・数量・実現損益・配当合計・[取引][削除]のみ。現在価格・評価額・前日比は出さない＝価格取得自体をしないため）。
- **日報API・cron記録の価格取得対象からも除外**：`GetPortfolioSnapshotCurrent`／`RecordSnapshots`（`MoneyBoardApi/DataApi.Portfolio.cs`）の Yahoo Finance／投信協会への価格取得ループも `PortfolioMath.IsHeld` で絞り込み、保有ゼロ銘柄への無駄なリクエストを削減。
- **売却済み判定は `PortfolioMath.IsHeld` に一本化**：画面（`Ordered`）だけでなく `GetPortfolioSnapshotCurrent` の `holdingInfos` 集計（旧 `sum.Quantity == 0` 判定）と `PortfolioMath.BuildSnapshot`（旧 `qty == 0` 判定）も `IsHeld` へ揃えた。判定基準がレイヤごとにバラバラだと、負数量（入力ミス）の銘柄が画面では除外される一方で日報API・推移スナップショットには古い価格のまま残ってしまい、同じ状態で総資産の値が画面／日報／推移履歴の間で食い違う不整合が生じるため（自動検証で実測・レビュー指摘）。
- **実現損益は生涯平均取得単価法（`PortfolioMath.Summarize`）による概算のため、買い戻すと過去の実現損益の表示値が変わる**（平均取得単価が買い戻し後の全買付履歴から再計算されるため）。これは #178 由来の新しい制約ではなく既存の `Summarize` の仕様（買い戻しシナリオで初めて表面化する）。正確な売却時点の実現損益を恒久的に保持する設計（売却時点でのスナップショット保存等）は本 issue のスコープ外。
...
### iOS PWA の safe-area 未適用修正（issue #149）
- mobile.css の `.botnav` には元々 `padding-bottom: env(safe-area-inset-bottom)` があったが、`index.html` の viewport meta に `viewport-fit=cover` が無いため iOS Safari/PWA では `env()` が常に 0 扱いになり、ホーム画面追加（standalone）時に下部ナビの左右端がノッチ機種の画面角丸に隠れていた。
- `index.html` の viewport meta に `viewport-fit=cover` を追加して `env(safe-area-inset-bottom)` を実効化（固定 px の底上げは機種ごとの角丸差に対応できないため不採用）。
- **フォローアップ（コードレビュー指摘・横持ち対応）**：`viewport-fit=cover` は bottom だけでなく left/right の safe-area も同時に実効化する（従来の暗黙 `contain` はノッチを避けて本文をレターボックス表示していた）ため、横持ち時にノッチ側の左右端へコンテンツが食い込む回帰リスクが生じる。本アプリは縦持ち専用ではなく回転可能（manifest に `orientation` ロック無し）。対応は2層に分ける：
  - **本文（ヘッダー含む）＝コンテナ `.wrap` に集約**：ヘッダー（`.home-head`/`.pf-head`/`.graph-header`）・一覧・グラフはすべて `.wrap`（`base.css`・水平 `16px`）配下のため、`.wrap` の水平パディングを `max(16px, env(safe-area-inset-left/right))` にして一括で inset。個別要素ごとに足す下策を避け、コンテンツ器の1点で吸収する（PC は `.app-shell.has-sidenav .wrap` が左右 `28px` に上書きするため影響なし・縦持ちは `env=0` で従来どおり `16px`）。
  - **bottom 固定の全幅バー＝背景を全幅のまま内側だけ inset**：`.botnav`／`BottomSheet.razor` の `.sheet-foot`／一括カテゴリの `.dd-action-bar` は `.wrap` の外にあり、かつ背景色は端まで敷きたいバーのため、バー自身に `padding-left`/`padding-right`（`.sheet-foot`/`.dd-action-bar` は既存水平 `16px` を下回らないよう `max(16px, env(...))`、`.botnav` はタブが `flex:1` で水平パディング元々無しのため素の `env(...)`）を付与し、タップ領域だけノッチ側から逃がす。
...
→ `MonthDoc`/`MonthReadDoc`（`DataApi.cs`・Cosmos保存/クエリ読み取り用ドキュメント）という同型の3〜4型構造を
持つ。設定（`settings`）と同じ理由・同じ対策のため、詳細は上の「設定にフィールドを追加するときの注意」を参照。
- **対応**: `MonthData` に永続化したいフィールドを追加したら、同名プロパティを **`MonthPart` と `MonthDoc`
  （クエリ読み取りに関わるなら `MonthReadDoc` も）** に追加する。変換コード（`DataApi.GetData`/`SaveData`・
  `AppStateStore.BuildMonthPart`・`StorageService.LoadAsync` の月次部分）は `ObjectSync.CopyMatchingProperties`
  に一本化済みのため、型に同名プロパティを追加するだけでよく、変換コード自体は触らなくてよい。
...
- **v7**（#77）: 財布（現金）機能。`Account.IsWallet`・`Ledger.WalletAtmDeposits`・`Debit.CategoryId` を追加。
  いずれも安全な default（false / 空リスト / null）を持つ加算的な追加のみで移行処理は不要。
- **v8**（#95）: 収入の固定費。`AppState.FixedIncomes`・`IncomeItem.IsFixed`/`IsVariable`/`AmountOverridden`/
  `FixedIncomeId` を追加。加算的な追加のみで移行処理は不要。
- **v9**（#111）: チュートリアル既読管理。`AppState.TutorialSeenVersion` を追加。加算的な追加のみで移行処理は不要。
- **v10**（#134）: ボーナス月設定。`AppState.BonusMonths` を追加。安全な default（`{6,12}`）を持つ加算的な
  追加のみで移行処理は不要。
- **v11**（#147）: 口座種別の enum 化。判定の正本を `Account.IsWallet`（bool）から `Account.Type`
  （`AccountType`）へ移し、既存の `IsWallet==true` を `Type=Wallet` へ変換する。
  **v5 以来はじめて実データを書き換える移行**である点に注意（v6〜v10 はすべて加算的追加）。
  - `Type` が設定済みの口座は上書きしない（旧フラグを残す設計のため、種別変更を巻き戻さないガード）。
  - `IsWallet` は削除せず、**`Type` の別名として書き戻しも維持する**（`AccountsTab.AddWallet` は両方を立てる）。
    全クライアントが v11 以降に揃うまでは、更新を保留した旧クライアント（`Type` を知らず `IsWallet` だけを
    見る）が同じデータを開いても財布判定を落とさないようにするため。旧フィールドの削除は全クライアントの
    移行完了後に別 issue で行う。
  - `AccountType` は数値で永続化されるため **append-only**（値の挿入・並べ替え禁止）。
- **v11 フォローアップ（#154）**: #147 が v5 以来はじめて実データを書き換える移行だったため、
  従来表面化しなかった2つの穴が見つかった。次にスキーマ版数を消費する変更（#148 等）の前に修正済み。
  - **版数ダウングレードの防止**: `SchemaMigration.Apply` は `state.SchemaVersion > CurrentVersion`
    （自分より新しいクライアントが書いた未来版数のデータを旧クライアントが開いた場合）には移行も
    版数の書き換えも行わず `false`（変更なし）を返す。旧実装は無条件に自版で上書きしていたため、
    v11 データを旧クライアントが開くと起動直後に版数を巻き戻して保存し、新フィールドが欠落した。
    **このガードは v11 に入っているため、次に効くのは v11→v12 以降の移行から**（v10→v11 自体は
    バグが配布済みバイナリ側にあり後から直せないため対象外）。
    - **限界**：ガードはクライアント側にしか無く、`SaveData` は受け取った `SchemaVersion` を検証せず
      そのまま保存する。したがって**配布済みの v10 以前のクライアントによる引き下げは今も防げない**。
      サーバー側で保存済み doc の `SchemaVersion` と比較して引き下げを拒否すれば既存クライアントにも
      即効く（**#155** で対応、詳細は下記フォローアップ）。
  - **サーバー側での正規化**: `DataApi.SaveData` は権威をフロント側（＝クライアントの `SchemaMigration`
    実行有無）に置かず、保存直前に `DataApi.NormalizeAccountTypes` で口座種別を収束させる。
    `{isWallet:true, type:EMoney}` のような自己矛盾したデータを送られても、`IsWallet` を `Type` と
    矛盾しない値（`Type==Wallet` のときのみ `true`）へ揃えて保存する。
    - **旧フラグからの復元（`IsWallet==true && Type==Normal` → `Type=Wallet`）は送信元が v11 未満の
      ときだけ**行う（`SchemaMigration.AccountTypeVersion` で判定）。`Type==Normal` は「通常口座」と
      「`Type` を知らないクライアントが送らなかった」の区別が付かないため、v11 以降にも無条件で
      適用すると **Wallet→通常口座 の変更を保存のたびに巻き戻す**（#148 の種別変更 UI で顕在化する）。
      復元規則の実体は `SchemaMigration.RestoreWalletTypeFromLegacyFlag`（クライアントの移行処理と
      サーバーの正規化で共有し、種別が増えたときに片方だけ直す事故を防ぐ）。
    - **限界**：この正規化で守れるのは Wallet だけ。`IsWallet` は bool のため Wallet 以外の種別を
      表現できず、**v11 未満のクライアントが設定を保存すると `Type=EMoney` 等は静かに Normal へ落ちる**
      （`type` が JSON から欠落し、旧フラグにも情報が無いため復元できない）。#148 以降、全クライアントが
      v11 以降に揃うまではこのリスクが残る。恒久対策は版数フロア＋「加算的追加でも必ず版数を上げる」
      運用への変更（上記の版数ダウングレードと同じ根）で、**#155** で対応する。
    - **当面の運用**：上記2つの限界はいずれも「旧クライアントが居残ること」が前提のため、
...
- **v11 フォローアップ②（#155）**: 版数フロア（`DataApi.SaveData` が保存済み doc より低い版数の保存を
  拒否）と、未知フィールド欠落対策の運用変更。
  - **比較対象はクライアントが理解できる版数**：フロアの比較対象は保存済み doc が持ち回る `SchemaVersion`
    ではなく、`DataEnvelope.ClientSchemaVersion`（＝送信元クライアントの `SchemaMigration.CurrentVersion`）
    にする。`SchemaMigration.Apply` は未来版数のデータに対して早期 return し（#154）`state.SchemaVersion`
    を巻き戻さないため、保存済み doc の版数で比較すると「新しいデータを開いただけの旧クライアント」が
    素通りしてしまう（設計時の詳細検討は issue #155 コメント参照）。
  - **ロールアウトは2段階**：`ClientSchemaVersion` は `int?`（nullable）。第1段（本リリース）では欠落
    （旧クライアント）を許可し保存を続ける（`DataApi.ViolatesSchemaFloor` はログのみで拒否しない）。
    デプロイ直後は開いたままのタブ・未更新PWAが必ず窓を作るため、非 nullable にすると一斉に保存不能へ
    陥る（int の既定値 `0` が常にフロアを下回るため）。第2段（`<ForceUpdate>` で旧クライアントを一掃した
    後のリリース）で欠落を拒否へ切り替える設計とし、その実施が下記（#174）。
    - **第2段切り替え完了（#174）**：保存済み doc が存在する場合、`ClientSchemaVersion` 欠落を
      `ViolatesSchemaFloor` が拒否するよう変更した（新規ユーザー＝保存済み doc 無しは従来どおり
      フロア対象外）。これにより v13 以降のフィールド追加リリースでフロアが実効化し、未知フィールド
      欠落対策（受入条件③の代替）が機能する。切り替えと同時に出荷するリリースは `<ForceUpdate>` を
      `true` にする必要がある（#142・切り替え前に旧クライアントを一掃しないと既存ユーザー全員が
      保存不能になるため）。
  - **RU コスト**：保存前に設定 doc の点読み取りが1回増える（≈1RU）。`SaveData` は従来 etag 条件付き
    更新のみで読み取りをしていなかったため純増だが、月次に比べて低頻度（設定変更時のみ）かつ点読みで
    軽量なため許容する。
  - **未知フィールド欠落（受入条件③）は JSON マージを実装せず、版数フロアと運用変更で防ぐ**：現状は
    加算的追加でも版上げを必須とする規則が無い（v2/v5〜v10 の移行不要な加算的変更がその例。版数自体は
    上がってきたが「毎回上げる」不変条件ではない）ため、同一 `SchemaVersion` 内でフィールド集合が
    食い違いうる（＝版数フロアだけでは欠落を防げない）。そこで **v13 以降、`SchemaVersion` を
    「フィールド集合の版」と定義し直し、加算的追加でも必ず上げる**運用に変更する（本 ADR で決定）。
    これにより版数フロアが効き、旧クライアントは保存自体を拒否されるため、フィールド単位の
    JSON レベルマージ（配列要素のマージまで要り複雑度が高くバグを埋め込みやすい）を実装せずに済む。
    代償は「フィールド追加のたびに全クライアントの更新が保存の前提になる」ことだが、招待ベータでは
    むしろ望ましい挙動であり `<ForceUpdate>`（#142）の導線も既にある。
  - **「必ず版上げ」の強制はコメント/ADR ではなくテストで行う**：この不変条件は人手の記憶に依存すると
    静かに破られる（フィールドを追加しても版上げを忘れればコンパイル・既存テストは通ってしまい、
    #155 が防いだはずの欠落が再発する）。`MoneyBoardShared.Tests.PersistedSchemaGuardTests` が
    `SettingsPart`/`MonthPart`（および参照先の型）の公開プロパティ集合を `SchemaSignature` で
    `SchemaMigration.CurrentVersion` ごとに凍結し、無断のフィールド追加を CI で検知する
    （フィールドを足したら版上げ＋新バージョンのシグネチャ追記が必須になり、既存バージョンの
    凍結値は出荷済みの記録として変更しない）。
  - **月次ドキュメントは対象外**：本 issue は設定ドキュメント優先で切った。月次（`MonthDoc`）への
    版数フロア適用は将来必要になれば別途検討する。
  - **見送った代替案（RDB 新設）**：Cosmos と RDB の使い分けも検討したが、欠落の根本原因は
    `DataApi.SaveData` のデシリアライズ時点（型無しフィールドが `default` になる）であり保存先の技術
    ではないため見送り。ストアを分割すると `TransactionalBatch` による原子性（設定＋月次の同時保存）が
    失われ、不整合の新規発生源になる点も理由（詳細は issue #155 コメント参照）。
