...
- **現行バージョン: `1.4.0`**（2026-06-21 本番リリース・PR #22。**Phase 4 土台＝Claude Vision でカード明細スクショをAI読み取り→当月へ取込**。詳細は「Phase 4」節）。`1.3.4`（2026-06-20・PR #21。CI のカバレッジを PR コメント＋Job Summary に出力＝テスト件数・備考・閾値の明示）／`1.3.3`（2026-06-20・PR #20。Step4 前クリーンアップ＝テスト基盤整備・純粋ロジック抽出・巨大razorの code-behind 分離＋楽天カード対応・表記統一）／`1.3.2`（2026-06-20・証券ポートフォリオ表示改善＋深いURL404修正＋認証永続化明示）／`1.3.1`（2026-06-19・スマホ実機フィードバック修正）／`1.3.0`=スマホUI全面最適化／`1.2.0`=Phase 3 証券ポートフォリオ。
...
```
main    本番デプロイ済み（直接pushしない）
dev     開発用（ここで作業）
```

### 作業フロー
1. `dev` で開発・push
2. GitHub で PR 作成 (dev → main)
3. GitHub Actions のビルド成功を確認
4. マージ → 自動デプロイ
...
C:\Development\moneyboard\
  MoneyBoard\
    Components\
      AccountsTab.razor       口座管理（マイページ内に内包・口座番号は廃止・cat-head形式）
      FixedCostTab.razor      固定費設定タブ（口座フィルター[Excel風複数選択]・D&D並び替え）
      MonthlyTab.razor        月次管理タブ（収入[給料/ボーナス/ATM入金/臨時収入]・支出[カード/ATM出金/手入力]・送金）
      CardTab.razor           カードタブ（明細の手入力/カードCSV取込[JCB/三井住友/PayPay/au PAY/楽天]/🤖AIで読取[スクショ]/一括カテゴリ・カードごと折りたたみ）
      MyPageTab.razor         マイページタブ（口座設定・カード明細カテゴリ・カード設定を折りたたみで内包）
  ...
  MoneyBoardShared\           ※ 役割は下記「MoneyBoardShared の憲章」を参照
    ...
    Portfolio.cs / PortfolioMath.cs  証券ポートフォリオのモデルと集計計算（Phase 3）。PortfolioMath に CostBasisJpyAsOf（指定日元本・円換算）/ YahooSymbol（日本株 .T 付与）を v1.3.3 で抽出
  MoneyBoardShared.Tests\     ※ xUnit(net8.0)。LedgerMath / LedgerEngine / PortfolioMath / StatsMath / FixedCostPeriod / CardCsvParser / Ym / SchemaMigration / FixedCost のユニットテスト（計102・`dotnet test`／カバレッジは `--collect:"XPlat Code Coverage"`）
```
...
- **テストプロジェクトは2つ**（いずれも xUnit・net8.0）：
  - `MoneyBoardShared.Tests`：`LedgerMath` / `LedgerEngine`（残高連鎖・ExpandCards・重複除外・固定費）/ `PortfolioMath`（集計・Valuation・CostBasisJpyAsOf・YahooSymbol）/ `StatsMath`（期間選択）/ `FixedCostPeriod`（年月の解析・整形）/ `CardCsvParser` / `Ym` / `SchemaMigration` / `FixedCost`（計**102**・v1.3.3 で 63→102）。
...
| マイページタブ（口座設定・カード明細カテゴリ・カード設定を折りたたみ内包） | ✅ 完了 |
...
- カード由来 💳 行は読み取り専用。クリックでカードタブの該当カードへ展開＋スクロール。
- 月ナビ横に ⓘ（`CycleInfo`）で当月サイクルの実期間を表示。
...
### マイページタブ
- **口座設定 / カード明細カテゴリ / カード設定** を折りたたみセクションで内包（既定は畳む）。
- 口座設定：口座名のみ（口座番号は廃止）・ボーナス受取は1つ・削除時は使用中チェックで警告。
- カード設定：削除は確認ダイアログ（消える明細件数・合計を明示）。

### カードタブ
- カードごとに折りたたみ（▼/▶＋カード名クリック）。たたむと件数・合計のみヘッダー表示。
- 一括カテゴリ：利用先グループにチェック→一括設定、カテゴリ絞り込み（未分類抽出）、未適用キャンセル時は破棄確認。
...
### カード明細管理 ✅
- カードマスタ（口座に紐づく）／カード専用タブで月ごとに管理
- 手入力＋カードCSV取込（JCB/三井住友/PayPay/au PAY/楽天。そのカードの当月を全置換）。`CardCsvParser` が種別ごとの列マッピング（日付列/利用先列/金額列）でパースし、日付として解釈できない行（ヘッダ/合計/情報行）は自動除外。文字コードは **PayPay と楽天が UTF-8(BOM可)、その他(JCB/三井住友/au PAY)が Shift-JIS**（ブラウザの `TextDecoder('shift_jis')` でデコード）。取込時は**過去月に既出の明細（リボ/分割の再掲）を除外**（`DedupAgainstEarlierMonths`）。※ 楽天は実CSV(enavi)で検証済み（列=[0]利用日/[1]利用店舗・商品名/[4]利用金額・UTF-8 BOM）
- 一括カテゴリ適用ダイアログ（利用先ごと）＋店名→カテゴリの自動分類ルール（`CategoryRules`）
- カード明細合計を月次の Debit(`CardId`付き) に展開
- ✅ スクリーンショット読み取り（Claude Vision・Phase 4 で実装）→「Phase 4」節を参照

### カテゴリ管理 ✅
- 12色・SortOrder・D&D並び替え（マイページ内）
- 未分類 = 灰色固定
- 一括カテゴリ適用／手入力・取込時の自動分類

### カテゴリ別支出グラフ ✅
- 統計ページにドーナツ＋一覧で追加。スライス色＝カテゴリ設定色
- 期間選択に連動・金額/割合/件数＋合計
- ドリルダウン明細一覧（モーダル）も実装済み（行クリック or スライス選択）
...
**形式（コミットも PR タイトルも共通）**: `type(scope): 日本語の要約`
- **type は必須**（PR タイトルにも必ず付ける。`Phase 2（…）` のような type 無しは禁止）。
- **scope は任意**（影響範囲。例: `auth` `card` `graph` `monthly` `fixed` `mypage` `ci` `deps`）。
- 要約は日本語・簡潔・末尾の句点なし。1 行目は概ね 50〜72 字以内、詳細は本文へ。
...
| perf | パフォーマンス改善 |
| docs | ドキュメント |
| style | 整形（コードの意味を変えない） |
| test | テスト |
| chore | 雑務・設定・依存更新 |
| ci | CI/CD |
| debug | 一時的な調査・計測（原因特定後に撤去する前提） |
| revert | リバート |

**本文 / PR Description は markdown**: `## 見出し`・`-` リスト・インライン `code`・必要に応じ `- [ ]` チェックボックス。PR 本文は「概要 → 変更点（種別ごと）→ デプロイ後の確認/フォロー → コミット一覧」を基本構成に。`debug`（一時計測）は本文に「原因確定後に撤去」と明記する。

**PR タイトル**: コミットと同じ `type(scope): 要約`。複数種別を束ねる PR は主要な変更の type を採用（機能追加が主なら `feat:`）。

**ツール注意**:
- コミットは **Bash の `git commit -m`** で書き、**本文は 1 つの `-m` に改行入りで渡す**（説明は `-` 箇条書きが段落に割れないように。件名と本文で `-m` を分ける）。PowerShell ヒアストリング `@'…'@` は区切りの `@` が件名/本文に漏れ、GitHub のタイトルが壊れる。
- メッセージ末尾に `Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>`。
