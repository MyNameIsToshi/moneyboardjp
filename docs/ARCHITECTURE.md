...
- **同名口座があると②口座別月末残高推移で例外が発生する不具合を修正（#157）**：`BuildChartData` が `Svc.ActiveAccounts.ToDictionary(a => a.Name, …)` で口座名をキーにしていたため、同名の口座が2つ以上あると `ArgumentException`（重複キー）が投げられ、`Load()` に try/catch が無いことから `OnInitializedAsync` が未処理のまま抜け、統計ページ全体が描画されなくなっていた。**修正**：キーを一意な `Account.Id` に変更（`StatsMath.BuildByAccountId`＝純粋関数として `MoneyBoardShared` 側に切り出し、`GraphPage.razor.cs` から呼び出す）。表示名（凡例のラベル）は `.razor` 側で `Svc.AccountName(series.Key)` を都度解決する。色割当（`BalanceLineOptions.Colors`）は元々 `ActiveAccounts` の並び順を直接参照しており、ディクショナリのキー変更の影響を受けないため無改修。口座名へのユニーク制約は、既存データに同名口座が存在しうるため採らない方針（受入条件どおり）。
