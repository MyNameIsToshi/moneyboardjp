...
C:\Development\moneyboard\
  ...
  MoneyBoardShared\           ※ 役割は下記「MoneyBoardShared の憲章」を参照
    ...
    StatsMath.cs              統計（グラフ）の純粋ロジック（SelectPeriodYms=期間選択。GraphPage が委譲・v1.3.3。NormalizeCategoryKey=カテゴリ別集計のグルーピングキー正規化・#117で追加。BuildAccountSeries=口座別系列の組み立て（Id 解決・順序保持・同名口座の表示名を連番で一意化）・#157で追加）
