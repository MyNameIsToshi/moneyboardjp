...
- **現金・電子マネーの手入力支出（`Debit`）を、明細ドリルダウンの日付表示・カテゴリ別月別推移でカード明細と同等に扱う（#171）**：`Debit` は実日付を持たず所属月 `ym` しか無いため、2箇所でカード明細（`CardDetail`）と扱いが不揃いだった。
  ...
  - **スコープ外**：月別推移の棒タップ由来の内訳ドリルダウン（`OpenCategoryTrendSummary`/`OpenTrendDetail`）は `CardDetailsAtBucket` のみを参照しており、現金・電子マネー分は含まれないまま（issue の想定変更どおり2ファイルに限定）。そのため棒の高さ（現金込み）とタップ後の内訳・明細合計（カードのみ）が一致しない場合がある。実装中に気付いた既知差異として記録（別 issue 化はユーザー判断）。
