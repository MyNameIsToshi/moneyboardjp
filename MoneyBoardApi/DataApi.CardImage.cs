...
public partial class DataApi
{
        ...
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "extract-card")] HttpRequest req)
    {
        ...
        {
            return HandleAnthropicUpstreamError(ex, "ExtractCard", "extract_card_error");
        }
        catch (Exception ex)
        {
            return HandleAnthropicGenericError(ex, "ExtractCard", "extract_card_error");
        }
    }
    ...
    private static async Task<List<CardDetail>> ExtractCardAsync(string base64Image, string? mediaType, string cardId)
    {
        ...
        var json = await CreateStructuredMessageAsync(CardImageSchema, 8000, content);
