...
public class FirebaseAuth
{
    ...
    public async Task<AuthPrincipal?> GetPrincipalAsync(HttpRequest req)
    {
        ...
        // SWA(マネージド関数)は Authorization ヘッダーを自前トークンで上書きするため、Firebase の
        // ID トークンは独自ヘッダー X-Firebase-Token で受け取る。無ければ Authorization: Bearer を
        // フォールバック（ローカル等 SWA を経由しない経路）として読む。
        string xfb = req.Headers["X-Firebase-Token"].ToString();
        string authz = req.Headers.Authorization.ToString();
        string token = !string.IsNullOrEmpty(xfb)
            ? xfb.Trim()
            : (authz.StartsWith("Bearer ", StringComparison.OrdinalIgnoreCase) ? authz["Bearer ".Length..].Trim() : "");
        string srcInfo = $"xfbLen={xfb.Length}, authzLen={authz.Length}";
        if (string.IsNullOrEmpty(token))
        {
            LastError = $"no-token ({srcInfo})";
            return null;
        }
        ...
        try
        {
            ...
            if (!result.IsValid)
            {
                LastError = await BuildDiagAsync(srcInfo, token, result.Exception);
        ...
        {
            LastError = await BuildDiagAsync(srcInfo, token, ex);
    ...
    private async Task<string> BuildDiagAsync(string srcInfo, string token, Exception? inner)
    {
        string tokenKid = "?";
        ...
        return $"{srcInfo}; tokLen={token.Length}; segs={token.Split('.').Length}; "
